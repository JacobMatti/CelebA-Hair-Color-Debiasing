...
train_base = CelebAHair("train", get_transform())
minority_idx = np.where((train_base.blond == 1) & (train_base.male == 1))[0]
all_idx = np.arange(len(train_base))
oversampled = np.concatenate([all_idx, np.tile(minority_idx, 29)])
np.random.shuffle(oversampled)

train_ds = CelebAHair("train", get_transform(augment=True), indices=oversampled)
...
for epoch in range(1, EPOCHS + 1):
    ...
    print(f"\nEpoch {epoch} of {EPOCHS} done — avg loss: {total_loss/len(train_loader):.4f}")
